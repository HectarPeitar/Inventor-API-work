...
| section 7 ST | fields 1–16 already conform (flange before web OK); profile code `I` recognized for HE/IPE members (regex extended 2026-09); flange/web thickness and radius geometrically derived from the start-face cross-section loop when Frame Generator parameters are absent (`GetProfileDimensionsFromGeometry`, VERIFIED 2026-09: plain HE 400 B → 24.00/13.50/0.00); 17–20 skew angles IMPLEMENTED + viewer-verified 2026-09 (sign convention: see blocks/ST.md) |
| section 8 BO | round-hole `t` field + trailing X-ref letter DONE Phase 2 (`face X[ref] Y O t`); slot width/height DONE Phase 3 — **width = centre-to-centre distance (supersedes Phase 3 "overall length", see F2)**, height=0.00; rectangle O/width/height/angle DONE Phase 3 (corner diameter, edge lengths, rotation); slot/rectangle angle face-frame mapping still PENDING; **sharp-corner rectangles: target viewer rejects BO `d=0.00` — IK internal contour is the verified working representation (viewer-compatibility rule, see `nc1/7th-edition/DSTV-KNOWLEDGE-MAP.md`)**; **rounded (filleted) rectangles: BO with `d` = 2×fillet-radius, IMPLEMENTED + VERIFIED 2026-09 (sketch fillets and 3D Fillet-tool fillets; the latter detected via the face inner edge loop)** |
| section 8 vs 12 | cut classifier must never route BO-describable holes to IK; viewer-verified exception: sharp-corner rectangular internal openings (BO `d=0.00` rejected by target viewer) — **IMPLEMENTED 2026-09: exporter routes confirmed sharp rectangles (4-line closed profile, 4 right angles from actual sketch geometry) to IK closed clockwise contours; rounded rectangles (4 lines + 4 arcs) remain skipped as before**; **filleted rectangles are BO-describable and must route to BO (sketch fillets: profile-path 4L+4A; 3D Fillet-tool: body inner-edge-loop probe)** |
| section 10 AK/IK | Phase 5: plates with theoretical dimensions, CCW external / CW internal, closed; **AK emission policy VERIFIED 2026-09: exporter emits AK for every member (unconditional `BuildAkBlocks`) — plain HE 400 B baseline (profile `I`, geometric section dims 400.00/300.00/24.00/13.50/0.00) accepted by target viewer with AK blocks and no warning; do not add suppression conditions without new viewer evidence** |
