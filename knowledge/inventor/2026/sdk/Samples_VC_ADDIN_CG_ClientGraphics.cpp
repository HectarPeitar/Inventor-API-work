...
BEGIN_MESSAGE_MAP(CClientGraphicsApp, CWinApp)
	//{{AFX_MSG_MAP(CClientGraphicsApp)
...
/////////////////////////////////////////////////////////////////////////////
// CClientGraphicsApp construction

CClientGraphicsApp::CClientGraphicsApp()
{
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CClientGraphicsApp object

CClientGraphicsApp theApp;
...
BOOL CClientGraphicsApp::InitInstance()
{
...
/*------------------------ CClientGraphicsFactory -----------------------------------*/

/*
 * This class factory is used by COM to create an object of class CClientGraphics
 * via its CreateInstance method. The CClientGraphics is the object with which Autodesk Inventor (R) establishes
 * initial contact using the IRxApplicationAddInServer interface that it MUST support.
 */

STDMETHODIMP CClientGraphicsFactory::CreateInstance(IUnknown* pUnkOuter, REFIID riid, void** ppv)
{
	...
	*ppv = NULL;
	...
	CRxClientGraphics* pClientGraphics = new CRxClientGraphics();
	OnErrorReturn(!pClientGraphics, E_OUTOFMEMORY);

	// Now QI for the requested interface and return as is.

	HRESULT hr = pClientGraphics->QueryInterface(riid, ppv);
	if (FAILED(hr))
		delete pClientGraphics;
	return hr;
}

STDMETHODIMP CClientGraphicsFactory::LockServer(BOOL fLock)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
STDMETHODIMP CClientGraphicsFactory::QueryInterface(REFIID riid, void** ppv)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
STDMETHODIMP_(ULONG) CClientGraphicsFactory::AddRef()
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
STDMETHODIMP_(ULONG) CClientGraphicsFactory::Release()
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
CClientGraphicsFactory::CClientGraphicsFactory()
{
	m_cRef = 0;
...
STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, void** ppv)
{
	...
	*ppv = NULL;
	...
	// check for supported clsid (only CLSID_ClientGraphics).
	//
	OnErrorReturn(!IsEqualCLSID(rclsid, CLSID_ClientGraphics), E_INVALIDARG);

	// Check for supported interfaces (only IClassFactory and IUnknown).
	//
	...
	*ppv = static_cast<IClassFactory*>(new CClientGraphicsFactory);
...
const TCHAR* g_RegTable[][3] =
{
	...
	{ _T("Software\\Classes\\CLSID\\{") CLSID_ClientGraphics_RegGUID  _T("}"), 0, CLSID_ClientGraphics_Name },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_ClientGraphics_RegGUID  _T("}\\Description"), 0, CLSID_ClientGraphics_Descr },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_ClientGraphics_RegGUID _T("}\\Implemented Categories"), 0, _T("Server has implemented these") },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_ClientGraphics_RegGUID  _T("}\\InprocServer32"), 0, DLLNAME_VALUE },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_ClientGraphics_RegGUID  _T("}\\InprocServer32"), _T("ThreadingModel"), _T("Apartment") },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_ClientGraphics_RegGUID  _T("}\\Settings"), 0, _T("Programmatically Configurable") },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_ClientGraphics_RegGUID  _T("}\\Settings"), _T("LoadOnStartUp"), _T("1") },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_ClientGraphics_RegGUID  _T("}\\Settings"), _T("Type"), _T("Standard") },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_ClientGraphics_RegGUID  _T("}\\Settings"), _T("SupportedSoftwareVersionGreaterThan"), SOFTWARE_VERSION_SUPPORTED },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_ClientGraphics_RegGUID  _T("}\\Settings"), _T("Version"), _T("1")},	
};
...
STDAPI DllRegisterServer()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr) && pCatReg)
		{
			...
			hr = pCatReg->RegisterClassImplCategories(CLSID_ClientGraphics, 1, &ImplCATID);
		}
...
STDAPI DllUnregisterServer()
{
	...
	{
		CATID ImplCATID = CATID_InventorVersionedApplicationAddIn;
		CATID ReqCATID = CATID_InventorApplicationAddInSite;
		...
		if (FAILED(pCatReg->UnRegisterClassImplCategories(CLSID_ClientGraphics, 1, &ImplCATID)))
			hr = E_FAIL;
	}
