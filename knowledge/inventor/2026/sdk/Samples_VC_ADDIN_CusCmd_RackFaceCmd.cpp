...
HRESULT CRackFaceCmd::InteractionEvents_OnHelp(EventTimingEnum BeforeOrAfter, NameValueMap *pContext, HandlingCodeEnum *pHandlingCode)
{
    ...
    hr = pHelpManager->DisplayHelpTopic(CComBSTR(_T("ADMAPI_16_0.chm")), CComBSTR(_T("")));
	if (FAILED(hr)) return hr;
...
HRESULT CRackFaceCmd::GetValueFromExpression(CString strExpression, double *dValue)
{
	...
	*dValue = 0.0;
	...
	hr = pUnitsOfMeasure->GetValueFromExpression(CComBSTR(strExpression), CComVariant(kLengthUnitsType) , &tempValue);

	if (FAILED(hr)) return hr;
	*dValue = tempValue.dblVal;
