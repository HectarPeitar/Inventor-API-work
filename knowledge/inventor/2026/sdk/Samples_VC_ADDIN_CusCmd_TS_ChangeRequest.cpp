...
HRESULT CChangeRequest::Execute(CChangeDefinition *pChangeDefinition, ChangeProcessorObject *pChangeProcessorObj)
{
	HRESULT hr;
	...
	//set the parent to get the call back when change processor terminates
	m_pChangeProcessor->SetParentDefinition(pChangeDefinition);

	//connect the change processor
	hr = m_pChangeProcessor->Connect(pChangeProcessorObj);
	if (FAILED(hr)) return hr;
