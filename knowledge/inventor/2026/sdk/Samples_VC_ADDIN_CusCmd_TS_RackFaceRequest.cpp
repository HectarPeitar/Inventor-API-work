...
const CString CRackFaceRequest::m_strNoTeethNodeName = L"NoTeeth";
const CString CRackFaceRequest::m_strRackHeightNodeName = L"RackHeight";
const CString CRackFaceRequest::m_strRackWidthNodeName = L"RackWidth";
const CString CRackFaceRequest::m_strRackExtentsNodeName = L"RackExtents";
const CString CRackFaceRequest::m_strRackFeatureExtentDirectionNodeName = L"RackFeatureExtentDirection";
const CString CRackFaceRequest::m_strRackFaceKeyNodeName = L"RackFaceKey";
const CString CRackFaceRequest::m_strRackEdgeKeyNodeName = L"RackEdgeKey";
...
//-----------------------------------------------------------------------------
HRESULT CRackFaceRequest::ChangeProcessorEvents_OnWriteToScript (Document *pDocument, NameValueMap *pContext, BSTR *pResultInputs)
{
    HRESULT hr;

	//get the change manager to generate the script keys
	CComPtr<ChangeManager> pChangeManager;
	hr = m_pApplication->get_ChangeManager(&pChangeManager);
	if (FAILED(hr)) return hr;

	//get the script key for the rack face
	CComBSTR bstrRackFaceScriptKey;
	hr = pChangeManager->GetScriptKeyFromObject(m_pRackFace, &bstrRackFaceScriptKey);
	if (FAILED(hr)) return hr;

	//get the script key for the rack edge
	CComBSTR bstrRackEdgeScriptKey;
	hr = pChangeManager->GetScriptKeyFromObject(m_pRackEdge, &bstrRackEdgeScriptKey);
	if (FAILED(hr)) return hr;

	//create the string to transcript out
	CString strScriptInputs;
	strScriptInputs.AppendFormat(_T("<%s>%d</%s>\n"), m_strNoTeethNodeName, m_iNoTeeth, m_strNoTeethNodeName);
	strScriptInputs.AppendFormat(_T("<%s>%lf</%s>\n"), m_strRackWidthNodeName, m_dRackWidth, m_strRackWidthNodeName);
	strScriptInputs.AppendFormat(_T("<%s>%lf</%s>\n"), m_strRackHeightNodeName, m_dRackHeight, m_strRackHeightNodeName);
	strScriptInputs.AppendFormat(_T("<%s>%lf</%s>\n"), m_strRackExtentsNodeName, m_dRackExtents, m_strRackExtentsNodeName);
	strScriptInputs.AppendFormat(_T("<%s>%ld</%s>\n"), m_strRackFeatureExtentDirectionNodeName, (int)(m_kRackFeatureExtentDirection), m_strRackFeatureExtentDirectionNodeName);
	strScriptInputs.AppendFormat(_T("<%s>%s</%s>\n"), m_strRackFaceKeyNodeName, bstrRackFaceScriptKey, m_strRackFaceKeyNodeName);
	strScriptInputs.AppendFormat(_T("<%s>%s</%s>"), m_strRackEdgeKeyNodeName, bstrRackEdgeScriptKey, m_strRackEdgeKeyNodeName);

	*pResultInputs = strScriptInputs.AllocSysString();
	
	return S_OK;
}

//-----------------------------------------------------------------------------
HRESULT CRackFaceRequest::ChangeProcessorEvents_OnReadFromScript (Document *pDocument, BSTR Inputs, NameValueMap *pContext)
{
	HRESULT hr;

	//read the parameters for replaying transcript
	m_iNoTeeth = _tstoi(GetNodeValue(Inputs, m_strNoTeethNodeName));
	m_dRackWidth = _tstof(GetNodeValue(Inputs, m_strRackWidthNodeName));
	m_dRackHeight = _tstof(GetNodeValue(Inputs, m_strRackHeightNodeName));
	m_dRackExtents = _tstof(GetNodeValue(Inputs, m_strRackExtentsNodeName));
	m_kRackFeatureExtentDirection = (PartFeatureExtentDirectionEnum)_tstoi(GetNodeValue(Inputs, m_strRackFeatureExtentDirectionNodeName));
	CComBSTR bstrRackFaceScriptKey = GetNodeValue(Inputs, m_strRackFaceKeyNodeName);
	CComBSTR bstrRackEdgeScriptKey = GetNodeValue(Inputs, m_strRackEdgeKeyNodeName);
	...
	//get the rack face
	CComPtr<IDispatch> pRackFaceDisp;
	hr = pChangeManager->GetObjectFromScriptKey(bstrRackFaceScriptKey, &pRackFaceDisp);
	if (FAILED(hr)) return hr;

	hr = pRackFaceDisp.QueryInterface(&m_pRackFace);
	if (FAILED(hr)) return hr;

	//get the rack edge
	CComPtr<IDispatch> pRackEdgeDisp;
	hr = pChangeManager->GetObjectFromScriptKey(bstrRackEdgeScriptKey, &pRackEdgeDisp);
	if (FAILED(hr)) return hr;

	hr = pRackEdgeDisp.QueryInterface(&m_pRackEdge);
	if (FAILED(hr)) return hr;

	return S_OK;
}

CString CRackFaceRequest::GetNodeValue(CString strScriptInputs, CString strNodeName)
{
	int iStartNodePos = strScriptInputs.Find(_T("<") + strNodeName + _T(">"));
	int iEndNodePos = strScriptInputs.Find(_T("</") + strNodeName + _T(">"));

	return strScriptInputs.Mid(iStartNodePos + strNodeName.GetLength() + 2, iEndNodePos - iStartNodePos - strNodeName.GetLength() - 2);
}
