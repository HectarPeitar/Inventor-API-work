...
BEGIN_MESSAGE_MAP(CSampleCommandApp, CWinApp)
	//{{AFX_MSG_MAP(CSampleCommandApp)
...
/////////////////////////////////////////////////////////////////////////////
// CSampleCommandApp construction

CSampleCommandApp::CSampleCommandApp()
{
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CSampleCommandApp object

CSampleCommandApp theApp;
...
BOOL CSampleCommandApp::InitInstance()
{
...
/*
 * This class factory is used by COM to create an object of class CSampleCommand
 * via its CreateInstance method. The CSampleCommand is the object with which Autodesk Inventor (R) establishes
 * initial contact using the IRxApplicationAddInServer interface that it MUST support.
 */

STDMETHODIMP CSampleCommandFactory::CreateInstance(IUnknown* pUnkOuter, REFIID riid, void** ppv)
{
	...
	*ppv = NULL;
	...
	CRxSampleCommand* pSampleCommand = new CRxSampleCommand();
	OnErrorReturn(!pSampleCommand, E_OUTOFMEMORY);

	// Now QI for the requested interface and return as is.
	HRESULT hr = pSampleCommand->QueryInterface(riid, ppv);
	if (FAILED(hr))
		delete pSampleCommand;
	return hr;
}

STDMETHODIMP CSampleCommandFactory::LockServer(BOOL fLock)
{
	AFX_MANAGE_STATE (AfxGetStaticModuleState());
...
STDMETHODIMP CSampleCommandFactory::QueryInterface(REFIID riid, void** ppv)
{
	AFX_MANAGE_STATE (AfxGetStaticModuleState());
...
STDMETHODIMP_(ULONG) CSampleCommandFactory::AddRef()
{
	AFX_MANAGE_STATE (AfxGetStaticModuleState());
...
STDMETHODIMP_(ULONG) CSampleCommandFactory::Release()
{
	AFX_MANAGE_STATE (AfxGetStaticModuleState());
...
CSampleCommandFactory::CSampleCommandFactory()
{
	m_cRef = 0;
...
STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, void** ppv)
{
	...
	*ppv = NULL;
	...
	// Check for support clsid.
	//
	OnErrorReturn(!IsEqualCLSID(rclsid, CLSID_SampleCommand), E_INVALIDARG);
	...
	*ppv = static_cast<IClassFactory*>(new CSampleCommandFactory());
...
const TCHAR* g_RegTable[][3] =
{
	...
	{ _T("Software\\Classes\\CLSID\\{") CLSID_SampleCommand_RegGUID _T("}"), 0, CLSID_SampleCommand_Name },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_SampleCommand_RegGUID _T("}\\Description"), 0, CLSID_SampleCommand_Descr },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_SampleCommand_RegGUID _T("}\\Implemented Categories"), 0, _T("Server has implemented these") },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_SampleCommand_RegGUID _T("}\\InprocServer32"), 0, DLLNAME_VALUE },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_SampleCommand_RegGUID _T("}\\InprocServer32"), _T("ThreadingModel"), _T("Apartment") },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_SampleCommand_RegGUID _T("}\\Settings"), 0, _T("Programmatically Configurable") },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_SampleCommand_RegGUID _T("}\\Settings"), _T("LoadOnStartUp"), _T("1") },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_SampleCommand_RegGUID _T("}\\Settings"), _T("Type"), _T("Standard") },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_SampleCommand_RegGUID _T("}\\Settings"), _T("SupportedSoftwareVersionGreaterThan"), SOFTWARE_VERSION_SUPPORTED },
	{ _T("Software\\Classes\\CLSID\\{") CLSID_SampleCommand_RegGUID _T("}\\Settings"), _T("Version"), _T("1")},	
};
...
STDAPI DllRegisterServer()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			hr = pCatReg->RegisterClassImplCategories (CLSID_SampleCommand, 1, &ImplCATID);
		}
	}
...
STDAPI DllUnregisterServer()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (FAILED(pCatReg->UnRegisterClassImplCategories (CLSID_SampleCommand, 1, &ImplCATID)))
			hr = E_FAIL;
	}
...
BEGIN_OBJECT_MAP(ObjectMap)
	OBJECT_ENTRY(CLSID_SelectEventHandler, CSelectEventHandler)
	OBJECT_ENTRY(CLSID_InteractionEventHandler, CInteractionEventHandler)
END_OBJECT_MAP()


int CSampleCommandApp::ExitInstance()
{
	_Module.Term();
...
BOOL CSampleCommandApp::InitATL()
{
