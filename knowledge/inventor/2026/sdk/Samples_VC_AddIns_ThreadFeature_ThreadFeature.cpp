...
BEGIN_MESSAGE_MAP(CThreadFeatureApp, CWinApp)
	//{{AFX_MSG_MAP(CThreadFeatureApp)
...
/////////////////////////////////////////////////////////////////////////////
// CThreadFeatureApp construction

CThreadFeatureApp::CThreadFeatureApp()
{
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CThreadFeatureApp object

CThreadFeatureApp theApp;
...
BOOL CThreadFeatureApp::InitInstance() 
{
...
/*
 * This class factory is used by COM to create an object of class CThreadFeature
 * via its CreateInstance method. The CThreadFeature is the object with which Autodesk Inventor (R) establishes 
 * initial contact using the IRxApplicationAddInServer interface that it MUST support.
 */

STDMETHODIMP CThreadFeatureFactory::CreateInstance (IUnknown* pUnkOuter, REFIID riid, void **ppv)
{
  AFX_MANAGE_STATE (AfxGetStaticModuleState());
  HRESULT Result=NOERROR;
  CRxThreadFeature *pThreadFeature=NULL;
  ...
  *ppv = NULL;
  ...
  pThreadFeature = new CRxThreadFeature(); 
  OnErrorState (!pThreadFeature, Result, E_OUTOFMEMORY, wrapup);

  // Now QI for the requested interface and return as is.
  
  Result = pThreadFeature->QueryInterface (riid, ppv);

wrapup:
  if (FAILED (Result) && pThreadFeature)
    delete pThreadFeature;

  return Result;
}

STDMETHODIMP CThreadFeatureFactory::LockServer (BOOL fLock)
{
  AFX_MANAGE_STATE (AfxGetStaticModuleState());
  if (fLock)
    IncrementDllLocks();
...
STDMETHODIMP CThreadFeatureFactory::QueryInterface (REFIID riid, LPVOID *ppv)
{
  AFX_MANAGE_STATE (AfxGetStaticModuleState());
  HRESULT Result=NOERROR;
...
STDMETHODIMP_(ULONG) CThreadFeatureFactory::AddRef()
{
  AFX_MANAGE_STATE (AfxGetStaticModuleState());
  ++m_cRef;
  return m_cRef;
...
STDMETHODIMP_(ULONG) CThreadFeatureFactory::Release()
{
  AFX_MANAGE_STATE (AfxGetStaticModuleState());
  if(! --m_cRef)
    delete this;
...
CThreadFeatureFactory::CThreadFeatureFactory ()
{
  m_cRef = 0;
...
STDAPI DllGetClassObject (REFCLSID rclsid, REFIID riid, LPVOID FAR* ppv)
{
  ...
  *ppv = NULL;
  ...
  // Else, return the class factory object of the specified class and reference count it. 
	...
	{
	  if (IsEqualCLSID (rclsid, CLSID_ThreadFeature))
	  {
      *ppv = new CThreadFeatureFactory;
		  if (!*ppv)
 ...
 */
#define SOFTWARE_VERSION_SUPPORTED _T("13..")

#define DLLNAME_VALUE (TCHAR*)-1
const TCHAR *g_RegTable[][3] = 
{
  // Format is: { Key, Value name, Value }
  { _T("Software\\Classes\\CLSID\\{") CLSID_ThreadFeature_RegGUID _T("}"), 0, CLSID_ThreadFeature_Name },
  { _T("Software\\Classes\\CLSID\\{") CLSID_ThreadFeature_RegGUID _T("}\\Description"), 0, CLSID_ThreadFeature_Descr },
  { _T("Software\\Classes\\CLSID\\{") CLSID_ThreadFeature_RegGUID _T("}\\Implemented Categories"), 0, _T("Server has implemented these") },
  { _T("Software\\Classes\\CLSID\\{") CLSID_ThreadFeature_RegGUID _T("}\\InprocServer32"), 0, DLLNAME_VALUE },
  { _T("Software\\Classes\\CLSID\\{") CLSID_ThreadFeature_RegGUID _T("}\\InprocServer32"), _T("ThreadingModel"), _T("Apartment") },
  { _T("Software\\Classes\\CLSID\\{") CLSID_ThreadFeature_RegGUID _T("}\\Settings"), 0, _T("Programmatically Configurable") },
  { _T("Software\\Classes\\CLSID\\{") CLSID_ThreadFeature_RegGUID _T("}\\Settings"), _T("LoadOnStartUp"), _T("1") },
  { _T("Software\\Classes\\CLSID\\{") CLSID_ThreadFeature_RegGUID _T("}\\Settings"), _T("Type"), _T("Standard") },
  { _T("Software\\Classes\\CLSID\\{") CLSID_ThreadFeature_RegGUID _T("}\\Settings"), _T("SupportedSoftwareVersionGreaterThan"), SOFTWARE_VERSION_SUPPORTED },
  { _T("Software\\Classes\\CLSID\\{") CLSID_ThreadFeature_RegGUID  _T("}\\Settings"), _T("Version"), _T("1")},    
};
...
STDAPI DllRegisterServer()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			hr = pCatReg->RegisterClassImplCategories(CLSID_ThreadFeature, 1, &ImplCATID);
		}
	}
...
STDAPI DllUnregisterServer()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (FAILED(pCatReg->UnRegisterClassImplCategories (CLSID_ThreadFeature, 1, &ImplCATID)))
			hr = E_FAIL;
	}
